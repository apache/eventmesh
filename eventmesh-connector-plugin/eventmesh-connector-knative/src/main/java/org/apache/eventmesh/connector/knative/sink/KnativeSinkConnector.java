package org.apache.eventmesh.connector.knative.sink;

import org.apache.eventmesh.connector.SinkConnector;

import java.nio.charset.StandardCharsets;


import java.util.List;
import java.util.Properties;
import io.cloudevents.CloudEvent;

import lombok.extern.slf4j.Slf4j;

@Slf4j
public class KnativeSinkConnector implements SinkConnector {
    private String sinkUrl;
    private Properties props;
    @Override
    public void init(Properties props) {
        this.props = props;
        sinkUrl = props.getProperty("connector.sinkUrl", "http://localhost:8080/sink");
    }
    @Override
    public void put(List<CloudEvent> events) {
        for (CloudEvent event : events) { try {
            java.net.HttpURLConnection conn = (java.net.HttpURLConnection) new java.net.URL(sinkUrl).openConnection();
            conn.setRequestMethod("POST"); conn.setDoOutput(true); conn.setRequestProperty("Content-Type", "application/cloudevents+json");
            conn.getOutputStream().write(event.getData() != null ? event.getData().toBytes() : new byte[0]);
            conn.getResponseCode(); conn.disconnect();
        } catch (Exception e) { log.warn("knative sink: {}", e.toString()); } }
    }
    @Override
    public void commit(List<CloudEvent> written) {

    }
}
