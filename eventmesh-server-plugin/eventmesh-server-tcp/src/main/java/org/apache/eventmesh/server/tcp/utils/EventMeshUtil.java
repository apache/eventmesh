...
public class EventMeshUtil {
    ...
    public static String printMqMessage(EventMeshMessage eventMeshMessage) {
        ...
        String keys = properties.get(TcpProtocolConstants.KEYS_UPPERCASE);
        if (!StringUtils.isNotBlank(keys)) {
            keys = properties.get(TcpProtocolConstants.KEYS_LOWERCASE);
        }

        String result = String.format("Message [topic=%s,TTL=%s,uniqueId=%s,bizSeq=%s]", eventMeshMessage.getTopic(),
                properties.get(TcpProtocolConstants.TTL), properties.get(TcpProtocolConstants.RR_REQUEST_UNIQ_ID), keys);
        return result;
    }
    ...
    public static String getMessageBizSeq(Message msg) {
        ...
        String keys = properties.getProperty(TcpProtocolConstants.KEYS_UPPERCASE);
        if (!StringUtils.isNotBlank(keys)) {
            keys = properties.getProperty(TcpProtocolConstants.KEYS_LOWERCASE);
        }
        return keys;
    }
    ...
    public static Message decodeMessage(EventMeshMessage eventMeshMessage) {
        ...
        for (final Map.Entry<String, String> entry : entries) {
            if (OMSUtil.isOMSHeader(entry.getKey())) {
