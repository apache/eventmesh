...
import org.apache.eventmeth.server.http.utils.OMSUtils;
...
public class AsyncHTTPPushRequest extends AbstractHTTPPushRequest {
    ...
    public void tryHTTPRequest() {
        ...
        handleMsgContext.getMsg().getUserProperties().put(HttpProtocolConstants.REQ_EVENTMESH2C_TIMESTAMP, String.valueOf(System.currentTimeMillis()));

        String content = "";
        try {
            content = new String(handleMsgContext.getMsg().getBody(), HttpProtocolConstants.DEFAULT_CHARSET);
        } catch (Exception ex) {
            return;
        }
        ...
        body.add(new BasicNameValuePair(PushMessageRequestBody.EXTFIELDS, JSON.toJSONString(OMSUtils.getMessageProp(handleMsgContext.getMsg()))));
        ...
        eventMeshHTTPServer.getMetricsServer().summaryMetrics.recordPushMsg();
        ...
        try {
            httpClientPool.getClient().execute(builder, new ResponseHandler<Object>() {
                ...
                public Object handleResponse(HttpResponse response) {
                    ...
                    eventMeshHTTPServer.getMetricsServer().summaryMetrics.recordHTTPPushTimeCost(cost);
                    if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
                        eventMeshHTTPServer.getMetricsServer().summaryMetrics.recordHttpPushMsgFailed();
                        ...
                        String res = "";
                        try {
                            res = EntityUtils.toString(response.getEntity(), Charset.forName(HttpProtocolConstants.DEFAULT_CHARSET));
                        } catch (IOException e) {
                            handleMsgContext.finish();
                            return new Object();
                        }
