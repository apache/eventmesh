...
package org.apache.eventmeth.server.http.factory;
...
public class SSLContextFactory {
    ...
    public static SSLContext getSslContext() {
        if (sslContextInstance == null) {
            synchronized (SSLContextFactory.class) {
                if (sslContextInstance == null) {
                    sslContextInstance = createSSLContext();
                }
            }
        }
    ...
    private static SSLContext createSSLContext() {
        ...
        try {
            ...
            keyStore.load(Files.newInputStream(Paths.get(Constants.EVENTMESH_CONF_HOME
                    + File.separator
