...
import org.apache.eventmesh.protocol.tcp.model.EventMeshTcpSendResult;
import org.apache.eventmesh.protocol.tcp.model.EventMeshTcpSendStatus;
...
public class MessageTransferTask extends AbstractTask {
    ...
    public MessageTransferTask(Package pkg, ChannelHandlerContext ctx, long startTime, EventMeshProtocolTCPServer eventMeshTCPServer) {
    ...
    private void addTimestamp(EventMeshMessage eventMeshMessage, Command cmd, long sendTime) {
        if (cmd.equals(RESPONSE_TO_SERVER)) {
            eventMeshMessage.getProperties().put(TcpProtocolConstants.RSP_C2EVENTMESH_TIMESTAMP, String.valueOf(startTime));
            eventMeshMessage.getProperties().put(TcpProtocolConstants.RSP_EVENTMESH2MQ_TIMESTAMP, String.valueOf(sendTime));
            eventMeshMessage.getProperties().put(TcpProtocolConstants.RSP_SEND_EVENTMESH_IP, CommonConfiguration.eventMeshServerIp);
        } else {
            eventMeshMessage.getProperties().put(TcpProtocolConstants.REQ_C2EVENTMESH_TIMESTAMP, String.valueOf(startTime));
            eventMeshMessage.getProperties().put(TcpProtocolConstants.REQ_EVENTMESH2MQ_TIMESTAMP, String.valueOf(sendTime));
            eventMeshMessage.getProperties().put(TcpProtocolConstants.REQ_SEND_EVENTMESH_IP, CommonConfiguration.eventMeshServerIp);
        }
    }
