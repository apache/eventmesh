...
public class BatchSendMessageV2Processor implements HttpRequestProcessor {
    ...
    public BatchSendMessageV2Processor(EventMeshProtocolHTTPServer eventMeshHTTPServer) {
    ...
    public void processRequest(ChannelHandlerContext ctx, AsyncContext<HttpCommand> asyncContext) throws Exception {
        ...
        if (!EventMeshHTTPConfiguration.eventMeshServerBatchMsgNumLimiter
                .tryAcquire(HttpProtocolConstants.DEFAULT_FASTFAIL_TIMEOUT_IN_MILLISECONDS, TimeUnit.MILLISECONDS)) {
            responseEventMeshCommand = asyncContext.getRequest().createHttpCommandResponse(
                    sendMessageBatchV2ResponseHeader,
            ...
            eventMeshHTTPServer.getMetricsServer().summaryMetrics.recordSendBatchMsgDiscard(1);
            asyncContext.onComplete(responseEventMeshCommand);
            return;
        }
        ...
        if (StringUtils.isBlank(sendMessageBatchV2RequestBody.getTtl()) || !StringUtils.isNumeric(sendMessageBatchV2RequestBody.getTtl())) {
            sendMessageBatchV2RequestBody.setTtl(String.valueOf(HttpProtocolConstants.DEFAULT_MSG_TTL_MILLS));
        }
...
//        Message rocketMQMsg = null;
        ...
        try {
            // body
            omsMsg.setBody(sendMessageBatchV2RequestBody.getMsg().getBytes(HttpProtocolConstants.DEFAULT_CHARSET));
            // topic
            ...
            if (!StringUtils.isBlank(sendMessageBatchV2RequestBody.getTag())) {
                omsMsg.putUserProperties(HttpProtocolConstants.TAG, sendMessageBatchV2RequestBody.getTag());
            }
            ...
            responseEventMeshCommand = asyncContext.getRequest().createHttpCommandResponse(
                    sendMessageBatchV2ResponseHeader,
                    SendMessageBatchV2ResponseBody.buildBody(EventMeshRetCode.EVENTMESH_PACKAGE_MSG_ERR.getRetCode(), EventMeshRetCode.EVENTMESH_PACKAGE_MSG_ERR.getErrMsg() + e.getLocalizedMessage()));
            asyncContext.onComplete(responseEventMeshCommand);
            return;
        }

        eventMeshHTTPServer.getMetricsServer().summaryMetrics.recordSendBatchMsg(1);

        SendMessageContext sendMessageContext = new SendMessageContext(sendMessageBatchV2RequestBody.getBizSeqNo(), omsMsg, batchEventMeshProducer, eventMeshHTTPServer);
        ...
        try {
            batchEventMeshProducer.send(sendMessageContext, new SendCallback() {
                ...
                public void onSuccess(SendResult sendResult) {
                    ...
                    eventMeshHTTPServer.getMetricsServer().summaryMetrics.recordBatchSendMsgCost(batchEndTime - batchStartTime);
            ...
            responseEventMeshCommand = asyncContext.getRequest().createHttpCommandResponse(
                    sendMessageBatchV2ResponseHeader,
                    SendMessageBatchV2ResponseBody.buildBody(EventMeshRetCode.EVENTMESH_SEND_BATCHLOG_MSG_ERR.getRetCode(), EventMeshRetCode.EVENTMESH_SEND_BATCHLOG_MSG_ERR.getErrMsg() + e.getLocalizedMessage()));
            asyncContext.onComplete(responseEventMeshCommand);
            long batchEndTime = System.currentTimeMillis();
            eventMeshHTTPServer.getHttpRetryer().pushRetry(sendMessageContext.delay(10000));
            eventMeshHTTPServer.getMetricsServer().summaryMetrics.recordBatchSendMsgCost(batchEndTime - batchStartTime);
