...
public class RequestProcessor implements RpcProcessor<RequestResponse> {
    ...
    @Override
    public void handleRequest(final RpcContext rpcCtx, final RequestResponse request) {
        final EventClosure closure = new EventClosure() {
            @Override
            public void run(Status status) {
                rpcCtx.sendResponse(getRequestResponse());
            }
        };
        this.metaService.handle(request, closure);
    }

    @Override
    public String interest() {
        return RequestResponse.class.getName();
    }
}
