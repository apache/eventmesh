...
public class HttpEventWrapper implements ProtocolTransportObject {
    ...
    public void buildSysHeaderForClient() {
        ...
        EnumSet<ProtocolKey.ClientInstanceKey> keyEnumSet = EnumSet.allOf(ProtocolKey.ClientInstanceKey.class);
        for (ProtocolKey.ClientInstanceKey instanceKey : keyEnumSet) {
            switch (instanceKey) {
                case BIZSEQNO:
                case UNIQUEID:
                    break;
                default:
                    sysHeaderMap.put(instanceKey.getKey(), headerMap.getOrDefault(instanceKey.getKey(), instanceKey.getValue()));
            }
        }
    }
