...
| Connector Runtime | **Beta** | 可用——端到端工作；自 [#5394](https://github.com/apache/eventmesh/pull/5394) 起 23 个插件全部完整实现（不再有模板桩），数据丢失加固见 [#5328](https://github.com/apache/eventmesh/pull/5328)；单元测试现已覆盖全部 23 个插件（webhook 类源/汇用本地 HTTP 密封测试，外部客户端类插件做契约级测试） | GA 标准由 #5296 架构 review 跟踪 |
