...
static int _wemq_proxy_cmp(const void* val1, const void* val2)
{
    ...
    if (proxy1->weight > proxy2->weight) return 1;
    if (proxy1->weight < proxy2->weight) return -1;
    return 0;
}
...
*/
static int _wemq_proxy_load_item(StWemqProxy* pproxy, const char* pstr, size_t len)
{
    char weight[64] = "";
    char idc[32] = "";
    char port[64] = "";
...
static int _wemq_proxy_load_by_http(StWemqProxy* pproxy, size_t size, const char* url, long timeout)
{
    ...
    LOGRMB(RMB_LOG_INFO, "_wemq_proxy_load_by_http:[resp:len=%u,%s]!", (unsigned int)req.len, req.data);
	if(req.len<=2)
		return -1;
	// 解析json字符串
        ...
        if (json == NULL)
        {
            LOGRMB(RMB_LOG_ERROR, "_wemq_proxy_load_by_http: json_tokener_parse error!");
            ret = -3;
            goto _LOAD_ITEM_END;
        }
        ...
        {
            LOGRMB(RMB_LOG_ERROR, "_wemq_proxy_load_by_http: json_object_object_get_ex error!");
            ret = -3;
        ...
        {
            LOGRMB(RMB_LOG_ERROR, "_wemq_proxy_load_by_http: wemqAccessServer is null");
        }
        json_object_put(wemq_proxy_servers);
        json_object_put(json);
    ...
    LOGRMB(RMB_LOG_INFO, "_wemq_proxy_load_by_http: [servers:%s]!", servers);
		...
		if (!json_object_object_get_ex(json, pRmbStConfig->cConsumerSysId, &wemq_proxy_servers))
        {
            LOGRMB(RMB_LOG_INFO, "current subsystem does not have special access list");
			 if(!json_object_object_get_ex(json, "wemqAccessServer", &wemq_proxy_servers))
        	{
            	LOGRMB(RMB_LOG_ERROR, "_wemq_proxy_load_by_http: json_object_object_get_ex error!");
           		ret = -3;
            	json_object_put(json);
    ...
    trim(servers);
    {
        ...
        while ((item = strtok_r(ptr, ";", &saveptr)) != NULL)
        {
            if (i >= (int)size)
            {
                LOGRMB(RMB_LOG_WARN, " [index: %d] [size: %zu] _wemq_proxy_load_by_http: servers too more!\n", i, size);
                break;
            }
            ...
            {
                LOGRMB(RMB_LOG_WARN, "[item: %s] _wemq_proxy_load_by_http:  _wemq_proxy_load_item error!\n", item);
            }
            ptr = NULL;
        }
        if (i == 0)
...
static int _wemq_proxy_save(StWemqProxy* pproxy, size_t size, const char* path)
{
    FILE *fp = NULL;
    int i = 0;
    ...
    {
        fprintf(fp, "%s:%u#%u\n", pproxy[i].host, pproxy[i].port, pproxy[i].weight);
    }
    fclose(fp);
    return 0;
}
...
int wemq_proxy_load_servers(const char* url, long timeout)
{
    ...
    //1. 通过HTTP请求配置中心
    ret = _wemq_proxy_load_by_http(tmp_wemq_proxy_list, WEMQ_PROXY_MAX_SIZE, url, timeout);
	if(ret <= 0 && (strstr(url,"wemqAccessServer") == NULL)){
		LOGRMB(RMB_LOG_WARN, "get wemq proxy departMent ip list failed,url=%s", url);
		return -1;
	}
	if (ret <= 0) {
    ...
    //2. 排序
    qsort(tmp_wemq_proxy_list, tmp_wemq_proxy_list_num, sizeof(StWemqProxy), _wemq_proxy_cmp);
    ...
    //打印配置中心和本地的配置
    for (i = 0; i < _wemq_proxy_list_num; i++) {
        LOGRMB(RMB_LOG_DEBUG, "[local address %d: host,%s|port,%u|idc:%s|weight,%u|index,%u|failed_time,%ld|flag,%d",
              i, _wemq_proxy_list[i].host, _wemq_proxy_list[i].port, _wemq_proxy_list[i].idc, _wemq_proxy_list[i].weight, _wemq_proxy_list[i].index, _wemq_proxy_list[i].failed_time, _wemq_proxy_list[i].flag);
    }
    for (j = 0; j < tmp_wemq_proxy_list_num; j++) {
        LOGRMB(RMB_LOG_DEBUG, "[configcenter address %d: host,%s|port,%u|idc:%s|weight,%u|index,%u|failed_time,%ld|flag,%d",
              j, tmp_wemq_proxy_list[j].host, tmp_wemq_proxy_list[j].port, tmp_wemq_proxy_list[j].idc, tmp_wemq_proxy_list[j].weight, tmp_wemq_proxy_list[j].index, tmp_wemq_proxy_list[j].failed_time, tmp_wemq_proxy_list[j].flag);
    }
    ...
    for (i = 0; i < _wemq_proxy_list_num; i++){
        if (_wemq_proxy_list[i].failed_time == 0 && _wemq_proxy_list[i].flag == 0)
            continue;
        for(j = 0; j < tmp_wemq_proxy_list_num; j++){
            if (strcmp(_wemq_proxy_list[i].host, tmp_wemq_proxy_list[j].host) == 0 && _wemq_proxy_list[i].port == tmp_wemq_proxy_list[j].port){
    ...
    memcpy(_wemq_proxy_list, tmp_wemq_proxy_list, sizeof(StWemqProxy) * tmp_wemq_proxy_list_num);
...
int wemq_proxy_get_server(char* host, size_t size, unsigned int* port)
{
    ...
    {
    	pthread_mutex_unlock(&__wemq_proxy_rmutex);
    	LOGRMB(RMB_LOG_ERROR, "get proxy list number is:%d", _wemq_proxy_list_num);
    	*port = 0;
        ...
        wemq_proxy_get_usable_server_list(wemq_proxy_usable_server_list, &wemq_proxy_usable_sever_weight, &wemq_proxy_usable_count);
		...
		gettimeofday(&tv, NULL);
        now_time = tv.tv_sec * 1000000 + tv.tv_usec;
        if ((_wemq_proxy_list[now_index].failed_time + WEMQ_PROXY_SERVER_MAX_TIME) <= now_time)
        {
   ...
   // } while (tmp != now_index);
    ...
    if (_wemq_proxy_list[now_index].failed_time != 0 &&
        (_wemq_proxy_list[now_index].failed_time + WEMQ_PROXY_SERVER_MAX_TIME) > now_time)
    {
    	pthread_mutex_unlock(&__wemq_proxy_rmutex);
    	LOGRMB(RMB_LOG_ERROR, "local proxy list all in black list");
    	//*port = pRmbStConfig->cWemqProxyPort;
...
//    long now_time = time(NULL);
    ...
    LOGRMB(RMB_LOG_INFO, "[used: host,%s|port,%u] [wemq_proxy_list_used:%d] wemq_proxy_goodbye!\n", host, port, _wemq_proxy_list_used);
    pthread_mutex_lock(&__wemq_proxy_rmutex);

    int i = 0;
    for (i = 0; i < _wemq_proxy_list_num; i++) {
    	if ((strcmp(host, _wemq_proxy_list[i].host) == 0) && (port == _wemq_proxy_list[i].port)) {
...
void wemq_proxy_get_usable_server_list(int* wemq_proxy_usable_server_list, int* wemq_proxy_usable_sever_weight, int* wemq_proxy_usable_count)
{
	if (pRmbStConfig->iWemqUseHttpCfg != 1) {
       return;
    }
	...
	memset(wemq_proxy_usable_server_list_backup,0x00,sizeof(int)*_wemq_proxy_list_num);
    ...
    for(; i < _wemq_proxy_list_num; i++){
		char isBlack = ((_wemq_proxy_list[i].failed_time + WEMQ_PROXY_SERVER_MAX_TIME) <= now_time)?'n':'y';
		...
		if(strcmp(_wemq_proxy_list[i].idc,pRmbStConfig->cRegion)==0){
        	...
        	if (_wemq_proxy_list[i].failed_time == 0 || (_wemq_proxy_list[i].failed_time + WEMQ_PROXY_SERVER_MAX_TIME) <= now_time){
			...
			if (_wemq_proxy_list[i].failed_time == 0 || (_wemq_proxy_list[i].failed_time + WEMQ_PROXY_SERVER_MAX_TIME) <= now_time){
            	...
            	wemq_proxy_usable_sever_weight_backup += (int)_wemq_proxy_list[i].weight;
            	//LOGRMB(RMB_LOG_DEBUG, "[host:%s | port:%u | weight:%d | index: %d] is usable!\n", _wemq_proxy_list[i].host, _wemq_proxy_list[i].port, _wemq_proxy_list[i].weight, i);
            	//LOGRMB(RMB_LOG_DEBUG, "j: %d | count: %d | weight: %d", j, *wemq_proxy_usable_count, *wemq_proxy_usable_sever_weight);
        	}
		}
	...
	if(!usableLocalIdc)
	{
		memset(wemq_proxy_usable_server_list, 0, sizeof(int)*_wemq_proxy_list_num);
		memcpy(wemq_proxy_usable_server_list, wemq_proxy_usable_server_list_backup, sizeof(int) * wemq_proxy_usable_count_backup);
