...
int GetRmbNowLongTime()
{
	...
	pRmbStConfig->ulNowTtime = (unsigned long)nowRmbTimeVal.tv_sec * (unsigned long)1000UL + (unsigned long)nowRmbTimeVal.tv_usec/1000UL;
	return 0;
}
...
char *RmbGetDateTimeStr(const time_t * mytime)
{
	...
	if(currRmb.tm_year > 50)
		snprintf(s, sizeof(s), "%04d-%02d-%02d %02d:%02d:%02d", currRmb.tm_year + 1900, currRmb.tm_mon + 1, currRmb.tm_mday, currRmb.tm_hour, currRmb.tm_min, currRmb.tm_sec);
	else
		snprintf(s, sizeof(s), "%04d-%02d-%02d %02d:%02d:%02d", currRmb.tm_year + 2000, currRmb.tm_mon + 1, currRmb.tm_mday, currRmb.tm_hour, currRmb.tm_min, currRmb.tm_sec);
	return s;
}
...
char *RmbGetShortDateStr(const time_t * mytime)
{
	...
	currRmb = *localtime(mytime);
	if(currRmb.tm_year > 50)
		snprintf(s, sizeof(s), "%04d%02d%02d", currRmb.tm_year + 1900, currRmb.tm_mon + 1, currRmb.tm_mday);
	else
		snprintf(s, sizeof(s), "%04d%02d%02d", currRmb.tm_year + 2000, currRmb.tm_mon + 1, currRmb.tm_mday);
...
int GetCurFileNum(FILE *fl)
{
	if (fl != NULL)
	{
		fscanf(fl, "%d %d %lu", &pStRmbLog->_curFileNo, &pStRmbLog->_curFileNums, &pStRmbLog->_lastShiftTime);
	}
	return 0;
}
...
int SetCurFileNum(FILE *fl)
{
	if (fl != NULL)
	{
		...
		rewind(fl);
		fprintf(fl, "%d %d %lu", pStRmbLog->_curFileNo, pStRmbLog->_curFileNums, pStRmbLog->_lastShiftTime);
	}
	return 0;
}
int GetCurFileNumFromStatFile()
{
	...
	fscanf(fp, "%d %d %lu", &pStRmbLog->_curFileNo, &pStRmbLog->_curFileNums, &pStRmbLog->_lastShiftTime);
	fclose(fp);
	return 0;
}
...
int ReloadCfg(int logLevel, int shiftType, const char* path, int maxFileSize, int maxFileNum)
{
	...
	snprintf(pStRmbLog->_logStatFileName, sizeof(pStRmbLog->_logStatFileName) - 1, "%s.stat", path);
	pStRmbLog->_para._maxFileSize = maxFileSize;
	pStRmbLog->_para._maxFileNum = maxFileNum;
	pStRmbLog->_lastShiftTime = time(NULL);
	if (strlen(pStRmbLog->_para._path) == 0 || pStRmbLog->_para._shiftType == RMB_LOG_TYPE_NORMAL)
	{
	...
	else if (pStRmbLog->_para._shiftType == RMB_LOG_TYPE_DAILY || pStRmbLog->_para._shiftType == RMB_LOG_TYPE_DAILY_AND_CYCLE)
	{
...
// 	printf("open file=%s succ", pStRmbLog->_logBaseFileName);
	pStRmbLog->_fd = open(pStRmbLog->_logBaseFileName, O_RDWR|  O_CREAT | O_APPEND, 0666);
	if (pStRmbLog->_fd == -1)
	{
		printf("openLog failed!fileName=%s, now select stdout!", pStRmbLog->_logBaseFileName);
		pStRmbLog->_fd = 1;
...
int FindNextDeleteLog(StDayInfo *dayInfo)
{
	...
	{
		pStRmbLog->_toDeleteFileNo = (pStRmbLog->_toDeleteFileNo + 1) % pStRmbLog->_para._maxFileNum;
		if (pStRmbLog->_toDeleteFileNo == 0)
		{
			pStRmbLog->_toDeleteFileNo = 1;
		}

		snprintf(pStRmbLog->_toDeleteFileName, sizeof(pStRmbLog->_toDeleteFileName) - 1, "%s_%s.%d", 
...
//				printf("pid=%d,getCurNo=%d,CurNums=%d", getpid(), pStRmbLog->_curFileNo, pStRmbLog->_curFileNums);
				...
				if (stat (pStRmbLog->_logBaseFileName, &pStRmbLog->gRmbSb) != -1 && (int)pStRmbLog->gRmbSb.st_size >= pStRmbLog->_para._maxFileSize)
				{
					pStRmbLog->_curFileNums += 1;
...
//						printf("pid=%d baseFileName=%s not exist", getpid(), pStRmbLog->_logBaseFileName);
					...
					pStRmbLog->_curFileNo = (pStRmbLog->_curFileNo + 1) % pStRmbLog->_para._maxFileNum;
...
//			printf("shift over!pid=%d, curNo=%d,curNum=%d, logFileName=%s", getpid(),  pStRmbLog->_curFileNo, pStRmbLog->_curFileNums,  pStRmbLog->_logFileName);
	...
	else if (pStRmbLog->_para._shiftType == RMB_LOG_TYPE_DAILY)
	{
			...
			if (-1 != fcntl(pStRmbLog->_fd, F_SETLKW, &pStRmbLog->logFileLock)) 
			{
				...
				if (-1 != fcntl(fd, F_SETLKW, &pStRmbLog->logStateLock)) 
				{
					...
					if (RmbIsOtherDay(&curTime, &pStRmbLog->_lastShiftTime))
					{
						...
						snprintf(pStRmbLog->_logBaseFileName, sizeof(pStRmbLog->_logBaseFileName), "%s_%s.log",
							pStRmbLog->_para._path, 
							RmbGetShortDateStr(&curTime)
							);
						snprintf(pStRmbLog->_logFileName, sizeof(pStRmbLog->_logFileName), "%s.0",
		...
		if (-1 != fcntl(pStRmbLog->_fd, F_SETLKW, &pStRmbLog->logFileLock)) 
		{
			...
			{
				GetCurFileNum(pStRmbLog->_pStatFile);
				snprintf(pStRmbLog->_logFileName, sizeof(pStRmbLog->_logFileName), "%s.%d",
					pStRmbLog->_logBaseFileName,
...
// 					}
	...
	else if (pStRmbLog->_para._shiftType == RMB_LOG_TYPE_DAILY_AND_CYCLE)
	{
		...
		if (RmbIsOtherDay(&curTime, &pStRmbLog->_lastShiftTime))
		{
			...
			snprintf(pStRmbLog->_logFileName, sizeof(pStRmbLog->_logFileName), "%s_1.log",
				pStRmbLog->_logBaseFileName
				);
			pStRmbLog->_curFileNo = 1;
			//OpenRmbLog();
		...
		rename(pStRmbLog->_logBaseFileName, pStRmbLog->_logFileName);
		pStRmbLog->_curFileNo += 1;
		snprintf(pStRmbLog->_logFileName, sizeof(pStRmbLog->_logFileName), "%s_%d.log",
			pStRmbLog->_logBaseFileName,
			pStRmbLog->_curFileNo
			);
		//OpenRmbLog();
...
// 	return ShiftRmbLog(pStRmbLog->_pLogFile);
	...
	fprintf(pStRmbLog->_pLogFile, "[%s][%s %03d][%d][%lu]", StatLogLevel[logLevel],RmbGetDateTimeStr((const time_t *) &(pStRmbLog->stLogTv.tv_sec)), (int) ((pStRmbLog->stLogTv.tv_usec)/1000),pRmbStConfig->uiPid,pthread_self());
...
int GetCurStateFromRmbLogByCirCle()
{
	...
	//int i = 0;
	snprintf(pStRmbLog->_logBaseFileName, sizeof(pStRmbLog->_logBaseFileName), "%s.log",
						pStRmbLog->_para._path
						);
...
// 	}
	...
	snprintf(pStRmbLog->_logFileName, sizeof(pStRmbLog->_logFileName) - 1, "%s.%d", pStRmbLog->_logBaseFileName, pStRmbLog->_curFileNo);
	snprintf(pStRmbLog->_toDeleteFileName, sizeof(pStRmbLog->_toDeleteFileName) - 1, "%s.%d", pStRmbLog->_logBaseFileName, pStRmbLog->_toDeleteFileNo);
...
//		printf("%d_baseName=%s", i , baseFileName);
		...
		{
			snprintf(pStRmbLog->_toDeleteFileName, sizeof(pStRmbLog->_toDeleteFileName) - 1, "%s.%d", baseFileName, i);
//			printf("check fileName=%s is exist or not!", pStRmbLog->_toDeleteFileName);
			struct stat sb;
			if (stat (pStRmbLog->_toDeleteFileName, &sb) != 0)
			{
...
//						printf("GetCurStateFromRmbLogByDaily logFile=%s has't exist", pStRmbLog->_toDeleteFileName);
	...
	snprintf(pStRmbLog->_toDeleteFileName, sizeof(pStRmbLog->_toDeleteFileName) - 1, "%s.1", baseFileName);
...
int GetDayInfo(StDayInfo* dayInfo, time_t* curTime)
{
	...
	snprintf(dayInfo->_baseFileName, sizeof(dayInfo->_baseFileName), "%s_%s.log",
		pStRmbLog->_para._path, 
		RmbGetShortDateStr(curTime)
		);
	...
	for (i = 1; i < pStRmbLog->_para._maxFileNum ; i++)
	{
		snprintf(dayInfo->_logFileName, sizeof(dayInfo->_logFileName) - 1, "%s.%d", dayInfo->_baseFileName, i);
...
int GetCurStateFromRmbLogByDaily_V3()
{
	...
	pStRmbLog->_toDeleteFileNo = dayInfo._deleteFileNo;
	pStRmbLog->_toDeleteTime = curTime;
	snprintf(pStRmbLog->_logBaseFileName, sizeof(pStRmbLog->_logBaseFileName), "%s_%s.log", pStRmbLog->_para._path, RmbGetShortDateStr(&curTime));
	snprintf(pStRmbLog->_logFileName, sizeof(pStRmbLog->_logFileName) - 1, "%s.%d", pStRmbLog->_logBaseFileName, pStRmbLog->_curFileNo);
...
//			printf("GetCurStateFromRmbLogByDaily logFile=%s not exist!", pStRmbLog->_logFileName);
	...
	for (i = 1; !finishFlag && i < pStRmbLog->_para._maxFileNum; i++ )
	{
		...
		snprintf(baseFileName, sizeof(baseFileName), "%s_%s.log",
			pStRmbLog->_para._path, 
			RmbGetShortDateStr(&uiDeleteTime)
			);
//		printf("%d_baseName=%s", i , baseFileName);
		int iNotNullFlag = 0;
		deleteNo = 0;
		...
		for (j = 1; j < pStRmbLog->_para._maxFileNum; j++)
		{
			snprintf(pStRmbLog->_toDeleteFileName, sizeof(pStRmbLog->_toDeleteFileName) - 1, "%s.%d", baseFileName, j);
//			printf("check fileName=%s is exist or not!", pStRmbLog->_toDeleteFileName);
			struct stat sb;
			if (stat (pStRmbLog->_toDeleteFileName, &sb) != 0)
			{
...
//						printf("GetCurStateFromRmbLogByDaily logFile=%s has't exist", pStRmbLog->_toDeleteFileName);
	...
	snprintf(pStRmbLog->_toDeleteFileName, sizeof(pStRmbLog->_toDeleteFileName) - 1, "%s.%d", baseFileName, pStRmbLog->_toDeleteFileNo);
//	printf("Status:curFileNums=%u,mayBeDeleteFileName=%s", pStRmbLog->_curFileNums, pStRmbLog->_toDeleteFileName);
	return 0;
}
...
void _Log_Thread_func(void* args)
{
	...
	unsigned long ulNowTime = (unsigned long)nowTimeVal.tv_sec * (unsigned long)1000UL + (unsigned long)nowTimeVal.tv_usec/1000UL;
	...
	while(1)
	{
		...
		if ((pRmbStConfig->iWemqUseHttpCfg == 1)) {
			...
			if (tv_now.tv_sec > (ulLastGetAccessTime + pRmbStConfig->getAccessIpPeriod)) {
				...
				LOGRMB(RMB_LOG_DEBUG, "config center ip str is:%s,try get access ip from the %d config center in list,lists:",tmpAddrs,pRmbStConfig->configIpPosInArr+1);
				...
				for (i = 0; i < lenAddrs; ++i)
				{
					...
					snprintf(cAccessIpUrl, sizeof(cAccessIpUrl), "%s/%s", addrArr[(i+pRmbStConfig->configIpPosInArr)%lenAddrs], WEMQ_ACCESS_SERVER);
					LOGRMB(RMB_LOG_DEBUG, "try to get access addr from %s",cAccessIpUrl);
					iRet = wemq_proxy_load_servers(cAccessIpUrl, pRmbStConfig->iConfigTimeout);
					
					if (iRet != 0) {
						LOGRMB(RMB_LOG_WARN, "get access ip from %s failed,try next config center ip", cAccessIpUrl);
						continue;
					} else {
						LOGRMB(RMB_LOG_INFO, "get all access ip list result=%d from url:%s", iRet, cAccessIpUrl);
