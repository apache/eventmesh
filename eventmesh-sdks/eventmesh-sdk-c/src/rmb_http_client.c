...
static int on_http_debug(CURL *handle, curl_infotype type, char *data, size_t size, void *userptr)
{
...
static size_t on_http_body(char *ptr, size_t size, size_t nmemb, void *userdata)
{
	...
	buf->data = (char *)realloc(buf->data, sizeof(char) * (buf->len + len + 1));
	if (buf->data == NULL) {
		/* out of memory */
