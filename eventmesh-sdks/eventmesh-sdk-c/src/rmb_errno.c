...
struct rmb_err_msg rmb_err_msg_array[] = {
	...
	{ RMB_ERROR_RR_INTERFACE_CAN_NOT_SEND_EVENT_MSG, "rr interface can't send event msg" },
	{ RMB_ERROR_EVENT_INTERFACE_CAN_NOT_SEND_RR_MSG, "event msg can't send rr msg" },
	...
	{ RMB_ERROR_RCV_MSG_APPHEADER_TOO_LARGE, "receive message appheader too large" },
...
void init_error()
{
	...
	{
		p_err_msg[rmb_err_msg_array[i].err_no - RMB_ERROR_BASE_BEGIN] =	rmb_err_msg_array[i].err_msg;
	}
	rmb_errno = 0;
}
...
const char* get_rmb_last_error()
{
	...
	if (rmb_errno <= RMB_ERROR_BASE_BEGIN && rmb_errno >= RMB_ERROR_BASE_BEGIN + RMB_MAX_ERR_NUMS)
	{
		return "unknown errorno";
	}
	return ((p_err_msg[rmb_errno - RMB_ERROR_BASE_BEGIN] == NULL) ? "unknown errorno" : p_err_msg[rmb_errno - RMB_ERROR_BASE_BEGIN]);
}
