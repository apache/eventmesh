...
int wemq_tcp_connect(const char *ip, uint16_t port, int timeout)
{
	...
	if (timeout > 0)
	{
		...
		if (getsockopt(sockfd, SOL_SOCKET, SO_ERROR,(char*)&sock_err, (socklen_t*)&sock_err_len) != 0)
		{
			close(sockfd);
			return -4;
		}
...
int wemq_tcp_recv(int fd, void *msg, uint32_t *len, int iTimeOut)
{
	...
	while (uiRemain > 0)
	{
		...
		{
			LOGRMB(RMB_LOG_ERROR, "Recv error, fd close() error=%d\n", errno);
			return -2;
		}
		if (iRecv == 0)
		{
	...
	if (uiRecved != 4 || strcmp(msg, "WEMQ") != 0) {
		LOGRMB(RMB_LOG_ERROR, "recv msg header error,%u:%s", uiRecved, (char *)msg);
		return -3;
	}
	...
	if (uiRecved != 4) {
		LOGRMB(RMB_LOG_ERROR, "recv msg version error,%u:%s", uiRecved, (char *)msg);
		return -3;
	}
	...
	while (uiRemain > 0)
	{
		...
		if (iRecv < 0 && (errno != EAGAIN))
		{
			LOGRMB(RMB_LOG_ERROR, "Recv error, fd close() errno=%d\n", errno);
			return -2;
		}
		if (iRecv == 0)
		{
...
void wemq_getsockename(int fd, char* ip, uint32_t len, int* port)
{
	...
	if (getsockname(fd, (struct sockaddr *)&guest, (socklen_t*)&guest_len) != 0)
	{
		return;
	}
...
void wemq_getpeername(int fd, char* ip, uint32_t len, int* port)
{
	...
	if (getpeername(fd, (struct sockaddr *)&svr, (socklen_t*)&svr_len) != 0)
	{
		return;
	}
