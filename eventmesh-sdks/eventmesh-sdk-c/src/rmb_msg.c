...
//private func
const static char *strLogicType[] =
  { "null", "req_in", "rsp_in", "event_in", "req_out", "rsp_out", "event_out",
"null" };
...
int rmb_msg_init (StRmbMsg * pRmbMsg, StRmbConfig * pConfig,
                  enum RMB_API_YPE type)
{
...
//      pRmbMsg->cApiType = C_TYPE;
  ...
  if (pRmbMsg->sysHeader.iSetSysVersionFlag != 1)
  {                             //没有被用户设置过
    RMB_MEMCPY (pRmbMsg->sysHeader.cConsumerSysVersion,
                pConfig->cConsumerSysVersion);
  }
...
int rmb_msg_set_consumerSysVersion (StRmbMsg * pRmbMsg,
                                    const char *cConsumerSysVersion)
{
  RMB_CHECK_POINT_NULL (pRmbMsg, "pRmbMsg");
...
int rmb_msg_set_dest (StRmbMsg * pRmbMsg, int desType, const char *cTargetDcn,
                      int iServiceOrEven, const char *cServiceId,
                      const char *cScenario)
{
  return rmb_msg_set_dest_v2 (pRmbMsg, cTargetDcn, cServiceId, cScenario);

}

int rmb_msg_set_dest_v2 (StRmbMsg * pRmbMsg, const char *cTargetDcn,
                         const char *cServiceId, const char *cScenario)
{
  return rmb_msg_set_dest_v2_1 (pRmbMsg, cTargetDcn, cServiceId, cScenario,
                                (char *) NULL);
}

int rmb_msg_set_dest_v2_1 (StRmbMsg * pRmbMsg, const char *cTargetDcn,
                           const char *cServiceId, const char *cScenario,
                           const char *cTargetOrgId)
{
  RMB_CHECK_POINT_NULL (pRmbMsg, "pRmbMsg");
  ...
  else
  {
    ...
    {
      LOGRMB (RMB_LOG_ERROR, "destination dcn must be 3 length!input=%lu",
              strlen (cTargetDcn));
      rmb_errno = RMB_ERROR_ARGV_LEN_ERROR;
      return -2;
    }
    strncpy (pRmbMsg->strTargetDcn, cTargetDcn,
             sizeof (pRmbMsg->strTargetDcn) - 1);
  }

  if (strlen (cServiceId) != DEFAULT_SERVICE_ID_LENGTH)
  {
    LOGRMB (RMB_LOG_ERROR,
            "destination service_id must be 8 length!input=%lu",
            strlen (cServiceId));
    rmb_errno = RMB_ERROR_ARGV_LEN_ERROR;
    return -2;
  }
  ...
  {
    LOGRMB (RMB_LOG_ERROR, "destination scene_id must be 2 length!input=%lu",
            strlen (cScenario));
    rmb_errno = RMB_ERROR_ARGV_LEN_ERROR;
    return -2;
  }

  pRmbMsg->iEventOrService =
    (*(cServiceId + 3) == '0') ? RMB_SERVICE_CALL : RMB_EVENT_CALL;

  strncpy (pRmbMsg->strServiceId, cServiceId,
           sizeof (pRmbMsg->strServiceId) - 1);
  strncpy (pRmbMsg->strScenarioId, cScenario,
           sizeof (pRmbMsg->strScenarioId) - 1);
  ...
  else
  {
    ...
    strncpy (pRmbMsg->strTargetOrgId, cTargetOrgId,
             sizeof (pRmbMsg->strTargetOrgId) - 1);
  }
  //modify 2016-09-26
...
//get cConsumerSysId
int rmb_msg_get_consumerSysId (StRmbMsg * pRmbMsg, char *cConsumerSysId,
                               unsigned int *uiLen)
{
  ...
  unsigned int uiConsumerLen =
    (unsigned int) strlen (pRmbMsg->sysHeader.cConsumerSysId);

  if (*uiLen < uiConsumerLen)
  {
    LOGRMB (RMB_LOG_ERROR,
            "rmb msg consumerSysId len=%u, but arg len=%u(too small)",
            uiConsumerLen, *uiLen);
    return -2;
  }
...
//get cConsumerSvrId
int rmb_msg_get_consumerSvrId (StRmbMsg * pRmbMsg, char *cConsumerSvrId,
                               unsigned int *uiLen)
{
  ...
  unsigned int uiConsumerLen =
    (unsigned int) strlen (pRmbMsg->sysHeader.cConsumerSvrId);

  if (*uiLen < uiConsumerLen)
  {
    LOGRMB (RMB_LOG_ERROR,
            "rmb msg consumerSvrId len=%u, but arg len=%u(too small)",
            uiConsumerLen, *uiLen);
    return -2;
  }
...
int rmb_check_msg_valid (StRmbMsg * pStMsg)
{
  ...
  if ((pStMsg->flag & (1 << RMBMSG_DEST_HAS_SET)) == 0)
  {
    LOGRMB (RMB_LOG_ERROR, "pStMsg=%s must be init first!",
            rmb_msg_print (pStMsg));
...
int rmb_msg_set_app_header (StRmbMsg * pRmbMsg, const char *appHeader,
                            unsigned int uiLen)
{
  RMB_CHECK_POINT_NULL (pRmbMsg, "pRmbMsg");
...
int rmb_msg_set_content (StRmbMsg * pRmbMsg, const char *content,
                         unsigned int uiLen)
{
  RMB_CHECK_POINT_NULL (pRmbMsg, "pRmbMsg");
...
const char *rmb_msg_get_app_header_ptr (StRmbMsg * pRmbMsg,
                                        unsigned int *pLen)
{
...
int rmb_msg_get_app_header (StRmbMsg * pRmbMsg, char *userHeader,
                            unsigned int *pLen)
{
  RMB_CHECK_POINT_NULL (pRmbMsg, "pRmbMsg");
  ...
  {
    LOGRMB (RMB_LOG_ERROR, " buffer len too small.uiLen=%u,buffSize=%u",
            *pLen, pRmbMsg->iAppHeaderLen);
    rmb_errno = RMB_ERROR_BUF_NOT_ENOUGH;
    return rmb_errno;
  }
...
int rmb_msg_get_content (StRmbMsg * pRmbMsg, char *content,
                         unsigned int *pLen)
{
  RMB_CHECK_POINT_NULL (pRmbMsg, "pRmbMsg");
  RMB_CHECK_POINT_NULL (content, "content");
  ...
  {
    LOGRMB (RMB_LOG_ERROR,
            " content buffer len too small.uiLen=%u,buffSize=%u", *pLen,
            pRmbMsg->iContentLen);
    rmb_errno = RMB_ERROR_BUF_NOT_ENOUGH;
    return rmb_errno;
  }
...
int shift_msg_2_buf (char *cBuf, unsigned int *pLen, const StRmbMsg * pStMsg)
{
  ...
  uiMsgLen =
    3 * sizeof (char) + sizeof (StSystemHeader) + 2 * sizeof (StDestination) +
...
static WEMQJSON *generate_destination_from_topic (StRmbMsg * pStMsg,
                                                  char *pTopic)
{
  ...
  json_object_object_add (jsonDest, MSG_BODY_DEST_NAME_STR,
                          json_object_new_string (cDestName));
  ...
  *pTemp = '\0';
  ...
  json_object_object_add (jsonDest, MSG_BODY_DEST_TYPE_STR,
                          json_object_new_string ("se"));
  json_object_object_add (jsonDest, MSG_BODY_DEST_SORE_STR,
                          json_object_new_string (strServiceId));
  json_object_object_add (jsonDest, MSG_BODY_DEST_SCENARIO_STR,
                          json_object_new_string (strScenarioId));
  json_object_object_add (jsonDest, MSG_BODY_DEST_DCN_STR,
                          json_object_new_string (strTargetDcn));
  json_object_object_add (jsonDest, MSG_BODY_DEST_ORGID_STR,
                          json_object_new_string (pStMsg->sysHeader.cOrgId));
...
int rmb_msg_print_v (StRmbMsg * pRmbMsg)
{
  LOGRMB (RMB_LOG_DEBUG,
          "[type=%s,send=%lu,rev=%lu,dest=%s,bizSeqNo=%s,cSeqNo=%s,replyTo=%s,len=%d,content=%s]",
          (int) pRmbMsg->cLogicType <=
          6 ? strLogicType[(int) (pRmbMsg->cLogicType)] : "null",
          pRmbMsg->sysHeader.ulSendTime, pRmbMsg->sysHeader.ulReceiveTime,
          pRmbMsg->dest.cDestName, pRmbMsg->sysHeader.cBizSeqNo,
          pRmbMsg->sysHeader.cConsumerSeqNo, pRmbMsg->replyTo.cDestName,
          pRmbMsg->iContentLen, pRmbMsg->cContent);
  return 0;
}

const char *rmb_msg_print (StRmbMsg * pRmbMsg)
{
  if (pRmbMsg->cLogicType == REQ_PKG_IN
      || pRmbMsg->cLogicType == EVENT_PKG_IN)
  {
    snprintf (pRmbMsg->strLogBuf, sizeof (pRmbMsg->strLogBuf) - 1,
              "[type=%s,send=%lu,rev=%lu,dest=%s,bizSeqNo=%s,cSeqNo=%s,replyTo=%s,len=%d,content=%s]",
              (int) pRmbMsg->cLogicType <=
              6 ? strLogicType[(int) (pRmbMsg->cLogicType)] : "null",
              pRmbMsg->sysHeader.ulSendTime, pRmbMsg->sysHeader.ulReceiveTime,
  ...
  else if (pRmbMsg->cLogicType == RSP_PKG_IN
           || pRmbMsg->cLogicType == RSP_PKG_OUT)
  {
    snprintf (pRmbMsg->strLogBuf, sizeof (pRmbMsg->strLogBuf) - 1,
              "[type=%s,send=%lu,rev=%lu,reply=%lu,dest=%s,bizSeqNo=%s,cSeqNo=%s,replyTo=%s,len=%d,content=%s]",
              (int) pRmbMsg->cLogicType <=
              6 ? strLogicType[(int) (pRmbMsg->cLogicType)] : "null",
              pRmbMsg->sysHeader.ulSendTime, pRmbMsg->sysHeader.ulReceiveTime,
              pRmbMsg->sysHeader.ulReplyTime, pRmbMsg->dest.cDestName,
              pRmbMsg->sysHeader.cBizSeqNo, pRmbMsg->sysHeader.cConsumerSeqNo,
              pRmbMsg->replyTo.cDestName, pRmbMsg->iContentLen,
              pRmbMsg->cContent);
  }
  else
  {
    snprintf (pRmbMsg->strLogBuf, sizeof (pRmbMsg->strLogBuf) - 1,
              "[type=%s,send=%lu,dest=%s,bizSeqNo=%s,cSeqNo=%s,replyTo=%s,len=%d,content=%s]",
              (int) pRmbMsg->cLogicType <=
              6 ? strLogicType[(int) (pRmbMsg->cLogicType)] : "null",
              pRmbMsg->sysHeader.ulSendTime, pRmbMsg->dest.cDestName,
              pRmbMsg->sysHeader.cBizSeqNo, pRmbMsg->sysHeader.cConsumerSeqNo,
              pRmbMsg->replyTo.cDestName, pRmbMsg->iContentLen,
              pRmbMsg->cContent);
  }
...
int rmb_msg_random_uuid (char *puuid, size_t size)
{
  ...
  uuid_unparse (uuidout, cUniqueIdEx);
