...
function get_pid {
	local ppid=""
	if [ -f ${DEMO_HOME}/bin/pid_tcp_pub.file ]; then
		ppid=$(cat ${DEMO_HOME}/bin/pid_tcp_pub.file)
	else
		if [[ $OS =~ Msys ]]; then
...
then
	...
	$JAVA $JAVA_OPT -classpath ${DEMO_HOME}/conf:${DEMO_HOME}/apps/*:${DEMO_HOME}/lib/* $DEMO_MAIN >> ${DEMO_LOG_HOME}/demo_tcp_pub.out 2>&1 &
echo $!>pid_tcp_pub.file
fi
exit 0
