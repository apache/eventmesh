...
function get_pid {
	local ppid=""
	if [ -f ${DEMO_HOME}/bin/pid_http_sub.file ]; then
		ppid=$(cat ${DEMO_HOME}/bin/pid_http_sub.file)
	else
		if [[ $OS =~ Msys ]]; then
...
echo "http_sub_demo use java location= "$JAVA
...
JAVA_OPT="${JAVA_OPT} -verbose:gc -Xloggc:${DEMO_HOME}/logs/demo_http_sub_gc_%p.log -XX:+PrintGCDetails -XX:+PrintGCDateStamps -XX:+PrintGCApplicationStoppedTime -XX:+PrintAdaptiveSizePolicy"
...
then
	...
	$JAVA $JAVA_OPT -classpath ${DEMO_HOME}/conf:${DEMO_HOME}/apps/*:${DEMO_HOME}/lib/* $DEMO_MAIN >> ${DEMO_LOG_HOME}/demo_http_sub.out 2>&1 &
echo $!>pid_http_sub.file
fi
exit 0
