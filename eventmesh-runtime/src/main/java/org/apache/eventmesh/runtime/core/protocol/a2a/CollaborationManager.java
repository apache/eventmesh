 ...
 * using EventMesh publish/subscribe infrastructure instead of point-to-point calls
 */
@Slf4j
public class CollaborationManager {
    ...
    public void initialize(EventMeshProducer eventMeshProducer) {
        ...
        log.info("CollaborationManager initialized with publish/subscribe service");
    }
    ...
    private boolean executeStep(CollaborationSession session, WorkflowStep step) {
        try {
            ...
            CompletableFuture<String> taskFuture = pubSubService.publishTask(taskRequest);
            ...
            taskFuture.whenComplete((taskId, throwable) -> {
                if (throwable == null) {
                    ...
                    log.info("Step {} published as task {}", step.getName(), taskId);
                } else {
                    log.error("Failed to publish step {}", step.getName(), throwable);
                    session.addStepContext(step.getName(), Map.of(
