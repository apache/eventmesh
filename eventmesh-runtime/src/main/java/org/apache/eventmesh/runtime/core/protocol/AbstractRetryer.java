...
public abstract class AbstractRetryer {
    ...
    protected final DelayQueue<Retryable> retrys = new DelayQueue<>();

    protected ThreadPoolExecutor pool;

    protected Thread dispatcher;

    protected abstract void pushRetry(RetryContext delayRetryable);
        ...
        dispatcher = new Thread(() -> {
            try {
                ...
                while (!Thread.currentThread().isInterrupted() && (retryObj = retrys.take()) != null) {
                    final Retryable delayRetryable = retryObj;
                    pool.execute(() -> {
                        try {
     ...
     */
    public DelayQueue<Retryable> getRetryQueue() {
        return retrys;
    }
}
