...
echo "using jdk[$JAVA]" >> ${PROXY_LOG_HOME}/defibus-proxy-client.out
...
ASYNC_REQ_MAIN=com.webank.emesher.client.sdkdemo.AsyncPublishInstance
$JAVA $JAVA_OPT -classpath ${PROXY_HOME}/conf:${PROXY_HOME}/lib/*:${PROXY_HOME}/apps/* $ASYNC_REQ_MAIN $conf $proxy $topic $packetsize  >> ${PROXY_LOG_HOME}/proxy-client.out 2>&1 &
exit 0
