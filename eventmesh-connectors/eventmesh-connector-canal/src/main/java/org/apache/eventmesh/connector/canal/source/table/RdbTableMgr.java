...
public class RdbTableMgr extends AbstractComponent {
    ...
    protected void startup() {
        if (config != null && config.getDatabases() != null) {
            for (RdbDBDefinition db : config.getDatabases()) {
                ...
                try (DruidDataSource dataSource = DatabaseConnection.createDruidDataSource(config.getUrl(),
                    ...
                    for (RdbTableDefinition table : db.getTables()) {
                        try {
                            ...
                            Map<String, List<String>> primaryKeys = queryTablePrimaryKey(dataSource, db.getSchemaName(),
                                    tables);
                            Map<String, List<MySQLColumnDef>> columns = queryColumns(dataSource, db.getSchemaName(), tables);
    ...
    private Map<String, List<String>> queryTablePrimaryKey(DruidDataSource dataSource, String schema,
    ...
    private Map<String, List<MySQLColumnDef>> queryColumns(DataSource dataSource, String schema, List<String> tables) throws SQLException {
        String prepareTables = SqlUtils.genPrepareSqlOfInClause(tables.size());
