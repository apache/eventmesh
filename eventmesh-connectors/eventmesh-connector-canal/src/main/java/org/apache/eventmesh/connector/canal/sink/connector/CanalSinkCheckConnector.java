...
@Slf4j
public class CanalSinkCheckConnector implements Sink, ConnectorCreateService<Sink> {
    ...
    private void init() {
        ...
        executor = new ThreadPoolExecutor(config.getParallel(), config.getParallel(), 0L, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<>(), new EventMeshThreadFactory("canal-sink-check"));
        List<CanalCheckConsumer> consumers = new LinkedList<>();
        for (int i = 0; i < config.getParallel(); i++) {
            CanalCheckConsumer canalCheckConsumer = new CanalCheckConsumer(queue, tableMgr, config);
